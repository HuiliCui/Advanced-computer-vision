...
class SMPLModel(Module):
    def __init__(self, device=None, model_path='./model_neutral.pkl',
        ...
        transl = torch.zeros([3],dtype=data_type).to(device)
        transl = torch.nn.Parameter(transl, requires_grad=True)
        self.register_parameter('transl', transl)

        scale = torch.ones([1],dtype=data_type).to(device)
    ...
    def forward(self, betas, thetas, transl=None, scale=None, gR=None):
        ...
        result = v + torch.reshape(transl, (batch_num, 1, 3))

        # estimate 3D joint locations
        joints = torch.tensordot(result, self.J_regressor, dims=([1], [1])).transpose(1, 2)
